...
ggplot(raw, aes(x = week, y = log10(`sjTREC QTY`))) +
    geom_point() +
    theme_bw()
...
ggplot(adata %>% filter(!is.na(CD4T)), aes(x = CD4T, y = log10TREC)) +
    geom_point() +
    theme_bw()

# try to reproduce some figures
ggplot(data, aes(x = week, y = log10TREC)) +
    geom_point() +
    theme_bw()
...
ggplot(cbind(data, tdat$predicted), aes(x = week, y = log10TREC)) +
    geom_point() +
    geom_point(aes(x = week, y = `tdat$predicted`, color = "red"))
rstatix::Anova(m)
performance::r2(m)
...
d.stats <- adata %>%
    t_test(log10TREC ~ sex, p.adjust.method = "fdr") %>%
    add_xy_position(x = "sex")

ggplot(adata, aes(x = sex, y = log10TREC)) +
    geom_boxplot() +
    geom_jitter() +
...
g1 <- ggplot(adata, aes(x = week, y = log10TREC, color = sex)) +
    geom_point() +
    geom_smooth()
d.stats <- adata %>%
    group_by(birth_group) %>%
    t_test(log10TREC ~ sex, p.adjust.method = "fdr") %>%
    add_xy_position(x = "birth_group")
g2 <- ggplot(adata, aes(x = birth_group, y = log10TREC, color = sex)) +
    geom_boxplot() +
    stat_pvalue_manual(d.stats, tip.length = 0.01, label = "p") +
    theme_bw()
ggarrange(g1, g2)
...
ggplot(adata %>% filter(!is.na(CD8T)), aes(x = CD8T, y = log10TREC)) +
    geom_point() +
    theme_bw()
...
ggplot(adata, aes(x = week, y = log10KREC)) +
    geom_point()
...
ggplot(tdat, aes(x = predicted, y = residual)) +
    ...
    geom_hline(yintercept = 0, lty = 3)
qqnorm(tdat$residual)
ggplot(cbind(data, tdat$predicted), aes(x = week, y = log10KREC)) +
    geom_point() +
    geom_point(aes(x = week, y = `tdat$predicted`, color = "red"))
rstatix::Anova(m)
performance::r2(m)
...
ggplot(adata %>% filter(!is.na(B.cells)), aes(x = B.cells, y = log10KREC)) +
    geom_point() +
    theme_bw()
